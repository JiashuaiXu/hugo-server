...
│              GitHub Actions (自动触发)                  │
│  ┌─────────────────────────────────┐    ┌─────────────┐ │
│  │ Enhanced Trigger Workflow       │───▶│ Build &     │ │
│  │ (obsidian-notes)                │    │ Deploy      │ │
│  │ • 触发 hugo-server 构建         │    │ (hugo-server) │ │
│  │ • 自动更新子模块引用            │    │             │ │
│  └─────────────────────────────────┘    └─────────────┘ │
│                                          │               │
└──────────────────────────────────────────┼───────────────┘
...
3. 自动更新 hugo-server 中的子模块引用，确保主仓库始终指向最新内容
4. hugo-server 拉取最新内容 → 构建 → 部署到 GitHub Pages

⏱️ **预计 2-5 分钟后**，新文章出现在 https://JiashuaiXu.github.io

💡 **重要说明**：现在您只需推送内容到 obsidian-notes 仓库，系统会自动处理主仓库的子模块引用更新，无需手动同步！
...
```yaml
name: Enhanced Trigger Hugo Build
...
jobs:
  trigger-hugo-build-and-update-submodule:
    runs-on: ubuntu-latest
    ...
    steps:
      ...
      - name: 更新 hugo-server 中的子模块引用
        run: |
          # 克隆 hugo-server 仓库
          git clone "https://x-access-token:${{ secrets.GH_PAT }}@github.com/JiashuaiXu/hugo-server.git" temp-hugo-server
          cd temp-hugo-server
          
          # 更新子模块引用
          cd jesse-blog/content
          git fetch origin
          git reset --hard ${{ steps.vars.outputs.commit_sha }}
          cd ../..
          ...
          if [[ -z $(git status --porcelain jesse-blog/content) ]]; then
            echo "子模块引用已是最新，无需更新"
          else
            git commit -m ":recycle: chore: 自动更新 content 子模块引用

更新到最新提交: ${{ steps.vars.outputs.commit_sha }}

由自动脚本触发，当 obsidian-notes 仓库更新时自动同步。"
...
**作用**：检测到内容变更 → 触发 hugo-server 构建 → 自动更新 hugo-server 中的子模块引用
