# 📚 文档索引

快速导航到所有项目文档。

## 核心文档

### [README.md](../README.md) ⭐
**主要文档** - 完整的系统架构、使用指南和故障排查

**内容包括**：
- 📊 系统架构图
- 🗂️ 仓库结构说明
- 🚀 快速开始指南
- 📝 日常使用流程
- ⚙️ 核心配置说明
- 🔧 GitHub Actions 详解
- 🐛 故障排查指南
- 🎯 最佳实践

### [SETUP_GUIDE.md](./SETUP_GUIDE.md) 🔧
**配置指南** - GitHub Actions 和 Secrets 的详细配置步骤

**内容包括**：
- 创建 GitHub Personal Access Token
- 配置仓库 Secrets
- 部署 workflows
- 使用流程说明
- 常见问题解答

### [功能文档索引](#功能文档索引) 📚
**功能文档** - 各个功能的详细实现说明

**内容包括**：
- [搜索功能实现](./01-search-feature.md)
- [阅读进度条实现](./02-reading-progress-bar.md) 
- [侧边目录实现](./03-sidebar-toc.md)
- [访客统计实现](./04-visitor-statistics.md)
- [自动化部署流程](./05-automated-deployment-workflow.md)
- [条件显示进度条](./06-conditional-progress-bar-display.md)
- [首页标签显示](./07-homepage-tags-display.md)
- [最近修复与改动总结（文章更新/构建失败/忽略目录）](./08-recent-fixes-and-updates.md)
- [一键发布脚本（本地自动提交 + 推送 + 更新子模块引用）](./09-publish-script.md)

### [obsidian-notes/README.md](../jesse-blog/content/README.md) 📝
**内容仓库文档** - Obsidian 集成和写作指南

**内容包括**：
- Obsidian 设置指南
- 写作和发布流程
- Front Matter 规范
- 图片管理
- Git 操作说明

## 快速查找

### 新手入门
1. [README.md - 快速开始](./README.md#-快速开始)
2. [SETUP_GUIDE.md - 配置步骤](./SETUP_GUIDE.md#配置步骤)
3. [README.md - 日常使用](./README.md#-日常使用)

### 功能文档
1. [01 - 搜索功能](./01-search-feature.md) 
2. [02 - 阅读进度条](./02-reading-progress-bar.md)
3. [03 - 侧边目录](./03-sidebar-toc.md)
4. [04 - 访客统计](./04-visitor-statistics.md)
5. [05 - 自动化部署](./05-automated-deployment-workflow.md)
6. [06 - 条件进度条](./06-conditional-progress-bar-display.md)
7. [07 - 首页标签](./07-homepage-tags-display.md)
8. [08 - 最近修复与改动总结](./08-recent-fixes-and-updates.md)
9. [09 - 一键发布脚本](./09-publish-script.md)

### 写作发布
1. [obsidian-notes/README.md - 写作指南](./jesse-blog/content/README.md#-写作指南)
2. [obsidian-notes/README.md - 发布流程](./jesse-blog/content/README.md#-发布流程)
3. [README.md - 文章 Front Matter](./README.md#文章-front-matter-模板)

### 故障排查
1. [README.md - 故障排查](./README.md#-故障排查)
2. [SETUP_GUIDE.md - 故障排查](./SETUP_GUIDE.md#故障排查)
3. [obsidian-notes/README.md - 故障排查](./jesse-blog/content/README.md#-故障排查)

### 高级配置
1. [README.md - GitHub Actions 详解](./README.md#-github-actions-详解)
2. [README.md - 依赖管理](./README.md#-依赖管理)
3. [README.md - 进阶技巧](./README.md#-进阶技巧)

## 外部资源

- **Hugo 官方文档**: https://gohugo.io/documentation/
- **PaperMod 主题**: https://github.com/adityatelange/hugo-PaperMod
- **GitHub Actions 文档**: https://docs.github.com/en/actions
- **Obsidian 官网**: https://obsidian.md

## 仓库链接

- **主仓库**: https://github.com/JiashuaiXu/hugo-server
- **内容仓库**: https://github.com/JiashuaiXu/obsidian-notes
- **部署网站**: https://JiashuaiXu.github.io

## GitHub Actions

- **hugo-server Actions**: https://github.com/JiashuaiXu/hugo-server/actions
- **obsidian-notes Actions**: https://github.com/JiashuaiXu/obsidian-notes/actions

---

**提示**：所有文档都经过精心编写，包含完整的使用说明和故障排查指南。遇到问题时，请先查阅相关文档。

**最后更新**: 2025-11-18
